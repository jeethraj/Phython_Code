...
#CC_EMAILS    = CC_EMAILS = ["saravanakannan.ganesan@tecnotree.com","kishor.reddy@tecnotree.com","jeethraj.poojary@tecnotree.com", "gunasekar.ravi@tecnotree.com"]
...
try:
    ...
    FROM (
        select SERVICE_IDENTIFIER_V AS CUSTOMER_NUMBER,
        
        CASE
            WHEN CALL_TYPE_V = '18' THEN 'DATA'
            WHEN CALL_TYPE_V = '001' THEN 'VOICE OUTGOING'
            WHEN CALL_TYPE_V = '002' THEN 'VOICE INCOMING'
            WHEN CALL_TYPE_V = '030' THEN 'SMS INCOMING'
            WHEN CALL_TYPE_V = '031' THEN 'SMS OUTGOING'
        END AS CALL_TYPE_LABEL
        ,COUNT(CALL_TYPE_V) AS CALL_COUNT
        from CB_PREPAID_UPLOAD_ALL_CDRS where  TRUNC(PROCESSING_DATE_TIME_DT)=TRUNC({date_input})
        group by  SERVICE_IDENTIFIER_V,CALL_TYPE_V)
    ...
    FROM (
        SELECT
        CHARGING_PARTY_NUMBER_V AS CUSTOMER_NUMBER,
        EVENT_TYPE_V,
        COUNT(EVENT_TYPE_V) AS EVENT_COUNT,
        CASE
            WHEN EVENT_TYPE_V = '40' THEN 'RECHARGE'
            WHEN EVENT_TYPE_V = '39' THEN 'BALANCE CHANGE'
            WHEN EVENT_TYPE_V = '6' THEN 'OFFER ACTIVATION'
            ELSE EVENT_TYPE_V
        END AS EVENT_TYPE_V_LABEL
    FROM CB_PREPAID_UPLOAD_ALL_EDRS
    WHERE  TRUNC(PROCESSING_DATE_TIME_DT)=TRUNC({date_input})
    GROUP BY CHARGING_PARTY_NUMBER_V, EVENT_TYPE_V
    )
    ...
    styled_html = df4.to_html(classes='table table-striped', index=False)
