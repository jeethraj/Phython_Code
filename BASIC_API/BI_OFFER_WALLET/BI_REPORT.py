...
try:
    ...
    mysql_rating_zone= """SELECT ZONE_ID_N, NETWORK_ID_N, DESCRIPTION_V, ZONE_TYPE_N
                        FROM RATING_ZONE_MASTER
                        WHERE STATUS_N = 1"""
    mysql_cursor.execute(mysql_rating_zone)
    rating_zone_Data = mysql_cursor.fetchall()

    

    columnsRatingzone = ["ZONE_ID_N","NETWORK_ID_N","DESCRIPTION_V","ZONE_TYPE_N"]
    df_ratingzone = pd.DataFrame(rating_zone_Data, columns=columnsRatingzone)

    csv_filename_Ratingzone= f"RatingZone_{date}.csv"
    downloads_folder = os.path.join(os.path.expanduser("~"), "C:\\Users\\poojaje\\Downloads\\OFFER_DUMP")
    csv_filename_path_ratingZone = os.path.join(downloads_folder, csv_filename_Ratingzone)


    df_ratingzone.to_csv(csv_filename_path_ratingZone, index=False)
    csv_filename_path = f"C:\\Users\\poojaje\\Downloads\\OFFER_DUMP\\{csv_filename_Ratingzone}"
    print("Rating Zone Generated Successfully")


    ## Network Identificaiton Master.

    mysql_NI= """SELECT IDENTIFICATION_ID_N, IDENTIFIER_VALUE_V, DESCRIPTION_V, IDENTIFICATION_TYPE_N
                        FROM NETWORK_IDENTIFICATION_MASTER
                        WHERE GUI_STATUS_N = 1"""
    mysql_cursor.execute(mysql_NI)
    mysql_NI_Data = mysql_cursor.fetchall()

    

    columnsNI = ["IDENTIFICATION_ID_N","IDENTIFIER_VALUE_V","DESCRIPTION_V","IDENTIFICATION_TYPE_N"]
    df_NI = pd.DataFrame(mysql_NI_Data, columns=columnsNI)

    csv_NI= f"NetworKIdentificationMaster_{date}.csv"
    downloads_folder = os.path.join(os.path.expanduser("~"), "C:\\Users\\poojaje\\Downloads\\OFFER_DUMP")
    csv_filename_path_NI = os.path.join(downloads_folder, csv_NI)


    df_NI.to_csv(csv_filename_path_NI, index=False)
    csv_filename_path = f"C:\\Users\\poojaje\\Downloads\\OFFER_DUMP\\{csv_NI}"
    print("Network Identificaiton Master Generated Successfully")
